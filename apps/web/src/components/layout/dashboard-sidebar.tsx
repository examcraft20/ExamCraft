...
export function DashboardSidebar() {
  ...
  const handleSignOut = async () => {
    try {
      ...
      localStorage.removeItem("examcraft_institution_id");
      localStorage.removeItem("examcraft_institution_name");
    ...
    <aside className="w-64 flex-shrink-0 bg-[#0f152d] flex flex-col sticky top-0 h-screen relative group/sidebar">
      ...
      <nav className="flex-1 px-4 pt-4 flex flex-col gap-1 overflow-y-auto pb-12 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        {navItems.map((item, iIdx) => {
          ...
          if ((item as any).isActive !== undefined)
      ...
      <div className="p-6 mt-auto">
        ...
        >
          <LogOut
            size={18}
            strokeWidth={2.5}
            className="text-[#f87171] group-hover/logout:scale-110 transition-transform"
          />
  ...
  switch (role) {
    case "faculty":
      return [
        {
          label: "Dashboard",
          href: `${base}${instId ? `?institutionId=${instId}` : ""}`,
          icon: Gauge,
        },
        {
          label: "Papers",
    ...
    case "academic_head":
      return [
        {
          label: "Overview",
          href: `${base}/overview${instId ? `?institutionId=${instId}` : ""}`,
          icon: Gauge,
        },
        {
          label: "Approvals",
          href: `${base}/approvals${instId ? `?institutionId=${instId}` : ""}`,
          icon: CheckSquare,
        },
        {
          label: "Analytics",
