...
export function PaperWorkspace({
  ...
  useEffect(() => {
    async function loadPapers() {
      try {
        const response = await apiRequest<PaperRecord[]>("/papers", {
          method: "GET",
          accessToken,
          institutionId,
        });
        ...
        setStatus(
          error instanceof Error
            ? error.message
            : "Unable to synchronize paper registry.",
        );
      } finally {
        setIsLoading(false);
      }
  ...
  async function handleSubmit(paperId: string) {
    try {
      await apiRequest(`/papers/${paperId}/submit`, {
        method: "PATCH",
        accessToken,
        institutionId,
      });
      ...
      setStatus(
        error instanceof Error ? error.message : "Submission protocol failed.",
      );
    }
  }

  async function handleDownloadPdf(paperId: string) {
    const url = `${env.apiUrl}/papers/${paperId}/pdf?institutionId=${institutionId}`;
  ...
  async function handleDownloadDocx(paperId: string) {
    const url = `${env.apiUrl}/papers/${paperId}/docx?institutionId=${institutionId}`;
    let downloadUrl: string | null = null;
    try {
      const response = await fetch(url, {
        headers: {
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-zinc-500 text-sm font-medium">
            Verify, coordinate, and export mission-critical examination assets.
          </p>
        </div>
        <div className="flex items-center gap-3 bg-zinc-900 border border-white/5 p-2 rounded-2xl">
          ...
          <div className="px-4 py-2 flex flex-col gap-0.5">
            ...
            <span className="text-sm font-bold text-indigo-400">
              {papers.filter((p) => p.status === "draft").length} Drafts
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="grid gap-6">
        {isLoading ? (
          ...
          <div className="p-32 rounded-[2.5rem] bg-white/5 border border-white/5 border-dashed text-center flex flex-col items-center gap-6 shadow-sm">
            ...
            <div className="flex flex-col gap-1">
              <h4 className="text-xl font-bold text-zinc-600 tracking-tight">
                Zero matching papers generated.
              </h4>
              <p className="text-zinc-500 text-sm font-medium">
                Head to &apos;Paper Templates&apos; to synchronize a new draft
                from an authorized template.
              </p>
            </div>
          </div>
        ) : (
          filteredPapers.map((paper) => (
            <div
              key={paper.id}
              className="group p-8 rounded-[2.5rem] bg-zinc-900 border border-white/5 hover:border-white/10 transition-all flex items-center justify-between gap-8 relative overflow-hidden shadow-xl"
            >
              <div
                className={`absolute top-0 right-0 w-32 h-32 blur-[60px] -z-10 opacity-0 group-hover:opacity-10 transition-all ${paper.status === "published" ? "bg-emerald-500" : paper.status === "draft" ? "bg-zinc-500" : "bg-indigo-500"}`}
              />

              <div className="flex items-center gap-6">
                <div
                  className={`w-14 h-14 rounded-2xl flex items-center justify-center border transition-all ${paper.status === "published" ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-400" : "bg-white/5 border-white/10 text-zinc-500 group-hover:text-indigo-400 group-hover:bg-white/10"}`}
                >
                  <FileText size={24} strokeWidth={2.5} />
                </div>
                <div className="flex flex-col gap-1">
                  <h4 className="text-xl font-black tracking-tight text-white group-hover:text-indigo-300 transition-colors uppercase decoration-zinc-700 underline-offset-4">
                    {paper.title}
                  </h4>
              ...
              <div className="flex items-center gap-4 opacity-0 group-hover:opacity-100 transition-all scale-95 group-hover:scale-100">
                ...
                <button
                  type="button"
                  aria-label="Open paper preview"
                  onClick={() => {
                    setSelectedPaper(paper);
                    setIsPreviewOpen(true);
                  }}
                  className="w-12 h-12 rounded-xl bg-white/5 border border-white/5 flex items-center justify-center text-zinc-600 hover:text-white hover:bg-white/10 transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500/50 focus:ring-offset-2"
                >
