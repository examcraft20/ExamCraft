...
import { Plus, AlertCircle, RefreshCw, UploadCloud, Lock, Search, Layers, FileUp, Trash2 } from "lucide-react";
...
export function QuestionListClient({
  ...
  const easy = questions.filter(q => q.difficulty === "Easy" || q.difficulty === "easy").length;
  const medium = questions.filter(q => q.difficulty === "Medium" || q.difficulty === "medium").length;
  const hard = questions.filter(q => q.difficulty === "Hard" || q.difficulty === "hard" || q.difficulty === "Difficult").length;

  const assignedSubjects = Array.from(new Set(questions.map(q => q.subject))).filter(Boolean);
  const assignedSubjectsText = assignedSubjects.length > 0 ? assignedSubjects.join(", ") : "None";

  const filteredQuestions = useMemo(() => {
    return questions.filter(q => {
      const searchMatch = q.title.toLowerCase().includes(filters.search.toLowerCase()) ||
                          q.subject.toLowerCase().includes(filters.search.toLowerCase());
      
      const subjectMatch = filters.subject === "All Subjects" || q.subject === filters.subject;
  ...
  const handleArchive = async (id: string) => {
    ...
    try {
      ...
      const instId = institutionId || localStorage.getItem("examcraft_institution_id");
      if (!instId) return;
  ...
  const getDifficultyColor = (diff: string) => {
    ...
    if (d === 'easy') return { bg: 'bg-emerald-500/10', text: 'text-emerald-400', border: 'border-emerald-500/20', dot: 'bg-emerald-400' };
    if (d === 'medium') return { bg: 'bg-amber-500/10', text: 'text-amber-400', border: 'border-amber-500/20', dot: 'bg-amber-400' };
  ...
  return (
    <div className="flex flex-col gap-6 w-full max-w-[1400px] mx-auto pb-20 mt-[-10px]">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-slate-400 text-sm font-medium mt-1">
            View and edit questions for your assigned subjects. <span className="text-white font-bold">({total} total)</span>
          </p>
        </div>
      ...
      <div className="flex items-center gap-2 px-4 py-3 rounded-lg border border-amber-500/20 bg-amber-500/10 text-amber-400 text-xs font-bold w-full shadow-sm">
        ...
        <span className="truncate">Showing questions for your assigned subjects only: {assignedSubjectsText}</span>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-white/5 border border-white/10 backdrop-blur-sm rounded-2xl p-5 hover:bg-white/[0.07] transition-all duration-200 flex flex-col justify-center gap-2 shadow-lg">
           <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                 <span className="text-[10px] uppercase font-black tracking-widest text-slate-500">Total Questions</span>
        ...
        <div className="bg-white/5 border border-white/10 backdrop-blur-sm rounded-2xl p-5 hover:bg-white/[0.07] transition-all duration-200 flex flex-col justify-center gap-2 shadow-lg">
           <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                 <span className="text-[10px] uppercase font-black tracking-widest text-slate-500">Easy</span>
        ...
        <div className="bg-white/5 border border-white/10 backdrop-blur-sm rounded-2xl p-5 hover:bg-white/[0.07] transition-all duration-200 flex flex-col justify-center gap-2 shadow-lg">
           <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                 <span className="text-[10px] uppercase font-black tracking-widest text-slate-500">Medium</span>
        ...
        <div className="bg-white/5 border border-white/10 backdrop-blur-sm rounded-2xl p-5 hover:bg-white/[0.07] transition-all duration-200 flex flex-col justify-center gap-2 shadow-lg">
           <div className="flex items-center gap-3">
              ...
              <div className="flex flex-col">
                 <span className="text-[10px] uppercase font-black tracking-widest text-slate-500">Difficult</span>
                  ...
                  return (
                    <tr key={q.id} className="hover:bg-slate-800/50 transition-colors group">
                       ...
                       <td className="px-6 py-5">
                         <span className="text-indigo-400 font-bold text-sm tracking-tight">{q.subject}</span>
                       </td>
                       <td className="px-6 py-5">
                       ...
                       <td className="px-6 py-5 text-center">
                          <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded border ${diffColor.bg} ${diffColor.border} ${diffColor.text} text-[10px] font-black uppercase tracking-widest`}>
                            {q.difficulty}
                          </div>
                       </td>
                       <td className="px-6 py-5 text-center">
