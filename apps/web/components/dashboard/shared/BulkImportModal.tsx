...
export function BulkImportModal({ isOpen, onClose, onSuccess, accessToken, institutionId }: BulkImportModalProps) {
  ...
  const validateAndSetPreview = (data: any[]) => {
    const validatedData = data.map((row): PreviewRow => {
      ...
      return {
        ...
        unitNumber: row.unit || row.Unit ? Number(row.unit || row.Unit) : undefined,
  ...
  const handleUpload = async () => {
    ...
    try {
      const formattedQuestions = validRows.map(r => ({
        ...
        courseOutcomes: r.courseOutcomes ? r.courseOutcomes.split(",").map(s => s.trim()) : [],
        status: "draft"
      }));
  ...
  const downloadTemplate = () => {
    const template = [
      ["title", "subject", "difficulty", "bloom_level", "marks", "unit", "tags", "course_outcomes"],
      ["What is the capital of France?", "General Knowledge", "easy", "Remember", "2", "1", "geography, europe", "CO1"],
      ["Explain the process of photosynthesis.", "Biology", "medium", "Understand", "5", "2", "plants, science", "CO2"]
    ];
      ...
      <div className="relative w-full max-w-4xl max-h-[90vh] bg-[#0f172a] border border-white/10 rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        <div className="p-8 pb-4 flex items-center justify-between border-b border-white/5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black text-white tracking-tight">Bulk Import Questions</h2>
              <p className="text-[#8b9bb4] text-sm font-medium">Upload CSV or XLSX to populate your question bank</p>
            </div>
          </div>
          <button 
            onClick={onClose}
          ...
          {!file ? (
            <div className="h-full flex flex-col items-center justify-center border-2 border-dashed border-white/10 rounded-[2rem] p-12 bg-white/[0.02] hover:bg-white/[0.04] transition-all group">
              ...
              <p className="text-[#8b9bb4] text-center max-w-sm mb-8">
                Supported formats: <strong className="text-white">.csv, .xlsx, .xls</strong>. Max size 10MB.
              </p>
              ...
              <div className="mt-12 grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-2xl">
                <div className="p-4 rounded-2xl bg-[#1e293b]/50 border border-white/5 flex gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold text-white uppercase tracking-wider mb-1">Requirements</p>
                    <p className="text-[11px] text-[#8b9bb4] leading-relaxed">
                      Columns <code className="text-indigo-300">title</code> and <code className="text-indigo-300">subject</code> are mandatory.
                    </p>
                  </div>
                </div>
                <div className="p-4 rounded-2xl bg-[#1e293b]/50 border border-white/5 flex gap-3">
                  ...
                  <div>
                    <p className="text-xs font-bold text-white uppercase tracking-wider mb-1">Formats</p>
                    <p className="text-[11px] text-[#8b9bb4] leading-relaxed">
                      Difficulty should be easy, medium, or hard. Tags should be comma-separated.
                    </p>
                  </div>
                </div>
              </div>
            ...
            <div className="h-full flex flex-col items-center justify-center p-20">
              ...
              <p className="text-white font-bold animate-pulse">Analyzing Spreadsheet Structure...</p>
            </div>
          ) : (
            <div className="flex flex-col gap-6">
              ...
              <div className="flex items-center justify-between p-5 rounded-3xl bg-white/5 border border-white/10">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-[#8b9bb4]">{(file.size / 1024).toFixed(1)} KB • {preview.length} rows detected</p>
                  </div>
                </div>
                <button 
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="bg-[#1e293b] rounded-2xl p-5 border border-white/5 shadow-xl">
                  <p className="text-[10px] font-black text-[#8b9bb4] uppercase tracking-widest mb-1">Valid Rows</p>
                  <p className="text-3xl font-black text-emerald-400">{validCount}</p>
                </div>
                <div className="bg-[#1e293b] rounded-2xl p-5 border border-white/5 shadow-xl">
                  <p className="text-[10px] font-black text-[#8b9bb4] uppercase tracking-widest mb-1">Issues Found</p>
                  <p className="text-3xl font-black text-amber-400">{invalidCount}</p>
                </div>
                <div className="bg-[#1e293b] rounded-2xl p-5 border border-white/5 shadow-xl">
                  <p className="text-[10px] font-black text-[#8b9bb4] uppercase tracking-widest mb-1">Total Items</p>
                  <p className="text-3xl font-black text-white">{preview.length}</p>
                </div>
              </div>
                ...
                <div className="max-h-[300px] overflow-y-auto">
                  <table className="w-full text-left text-xs">
                    ...
                    <tbody className="divide-y divide-white/5">
                      {preview.map((row, i) => (
                        <tr key={i} className={`hover:bg-white/[0.02] ${!row.isValid ? 'bg-amber-500/5' : ''}`}>
                            ...
                            {row.isValid ? (
                              ...
                              <div className="group relative">
                                ...
                                <div className="absolute left-6 top-0 hidden group-hover:block z-50 bg-[#0f172a] border border-white/10 p-2 rounded-lg shadow-2xl w-48 font-medium text-amber-200">
                                  {row.errors.map((e, ei) => <div key={ei} className="mb-1">• {e}</div>)}
                                </div>
                              </div>
                            )}
                          </td>
                          <td className="px-5 py-3 text-[#cbd5e1] font-medium max-w-[200px] truncate">{row.title}</td>
                          <td className="px-5 py-3 text-indigo-400 font-bold">{row.subject}</td>
                          <td className="px-5 py-3 text-[#8b9bb4] capitalize">{row.difficulty || "—"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        <div className="p-8 pt-4 flex items-center justify-between border-t border-white/5 bg-white/[0.01]">
          <p className="text-xs text-[#8b9bb4] font-medium">
            {preview.length > 0 && `${validCount} questions ready for ingestion`}
          </p>
          <div className="flex gap-4">
            ...
            >
              {isUploading ? <Spinner size="sm" className="mr-2" /> : <Sparkles size={16} className="mr-2" />}
