-- 3A. Paper status change trigger
CREATE OR REPLACE FUNCTION audit_paper_status_changes()
RETURNS TRIGGER AS $$
BEGIN
  IF OLD.status IS DISTINCT FROM NEW.status THEN
    INSERT INTO audit_logs (institution_id, user_id, action, resource_type, resource_id, metadata)
    VALUES (
...
CREATE TRIGGER trg_audit_paper_status
  AFTER UPDATE ON institution_papers
  FOR EACH ROW EXECUTE FUNCTION audit_paper_status_changes();

-- 3B. User role change trigger
...
BEGIN
  -- We need institution_id. We fetch it from institution_users matching the id
...
CREATE TRIGGER trg_audit_user_role_changes
  AFTER INSERT OR UPDATE OR DELETE ON institution_user_roles
  FOR EACH ROW EXECUTE FUNCTION audit_user_role_changes();

-- 3C. Question deletion trigger
...
CREATE TRIGGER trg_audit_question_deletion
  AFTER DELETE ON institution_questions
  FOR EACH ROW EXECUTE FUNCTION audit_question_deletion();
