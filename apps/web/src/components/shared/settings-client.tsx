...
import { Settings, Search, User, Lock, Bell } from "lucide-react";
  ...
  const CustomToggle = ({
    ...
    <button
      ...
      className={`w-11 h-6 rounded-full flex items-center px-1 transition-colors ${
        checked ? "bg-[#7c3aed]" : "bg-slate-700"
      }`}
    >
      <div
    ...
    <div className="flex flex-col w-full max-w-[900px] gap-8 mx-auto pb-20 mt-[-10px] text-slate-300">
      ...
      <div className="flex items-center gap-4 p-5 rounded-2xl bg-[#1e293b] border border-[#2b3952] shadow-sm">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-sm font-medium text-slate-400">
            {email || "No email"} {" | "}{" "}
            <span className="text-indigo-400">{role || "No role"}</span>
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-col gap-8">
        <div className="flex flex-col rounded-2xl bg-[#1e293b]/90 border border-[#2b3952] shadow-sm p-6 gap-6">
          <div className="flex items-center gap-3 border-b border-[#2b3952]/50 pb-4">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-xs text-slate-400">
                Your name and default context for generating papers.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-5">
            <div className="flex flex-col gap-2">
              ...
              <input
                ...
                className="h-11 rounded-lg bg-[#0f172a]/50 border border-slate-700/60 px-4 text-sm text-slate-300 focus:outline-none focus:border-indigo-500/50"
              />
            </div>
          ...
          <div>
            <Button className="bg-[#7c3aed] hover:bg-[#6d28d9] px-6 text-white font-bold h-10 shadow-[0_0_15px_rgba(124,58,237,0.2)]">
              Save Profile
            </Button>
          </div>
        </div>
        ...
        <div className="flex flex-col rounded-2xl bg-[#1e293b]/90 border border-[#2b3952] shadow-sm p-6 gap-6">
          ...
          <div>
            <Button className="bg-[#4338ca] hover:bg-[#3730a3] px-6 text-white font-bold h-10 shadow-none border border-indigo-500/20">
              Update Password
            </Button>
          </div>
        </div>
        ...
        <div className="flex flex-col rounded-2xl bg-[#1e293b]/90 border border-[#2b3952] shadow-sm p-6 gap-6">
          <div className="flex items-center gap-3 border-b border-[#2b3952]/50 pb-4">
            ...
            <div className="flex flex-col">
              ...
              <p className="text-xs text-slate-400">
                Control which in-app notifications you see on your dashboard.
              </p>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-slate-500">
                  Show a notice when your submitted question is approved by
                  admin.
                </span>
              </div>
              <CustomToggle
                checked={notifyApproved}
                onChange={() => setNotifyApproved(!notifyApproved)}
              />
            </div>
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-slate-500">
                  Show a notice when a submission is rejected with a review
                  note.
                </span>
              </div>
              <CustomToggle
                checked={notifyRejected}
                onChange={() => setNotifyRejected(!notifyRejected)}
              />
            </div>
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                ...
                <span className="text-xs text-slate-500">
                  Occasional tips about improving question quality and coverage.
                </span>
              </div>
              <CustomToggle
                checked={notifyTips}
                onChange={() => setNotifyTips(!notifyTips)}
              />
            </div>
          </div>
        ...
        <div className="flex flex-col rounded-2xl bg-[#1e293b]/90 border border-[#2b3952] shadow-sm p-6 gap-6">
          <div className="flex items-center gap-3 border-b border-[#2b3952]/50 pb-4">
            <div className="bg-blue-500 text-white rounded p-1 w-6 h-6 flex items-center justify-center font-bold font-serif text-sm">
              i
            </div>
            <div className="flex flex-col">
              <h2 className="text-lg font-bold text-white tracking-tight">
              ...
              <p className="text-xs text-slate-400">
                These fields are managed by your institution administrator.
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col gap-5 text-slate-500">
            <div className="grid grid-cols-2 gap-4">
              <div className="flex flex-col gap-2">
                ...
                <input
                  ...
                  className="h-11 rounded-lg bg-[#0f172a]/30 border border-slate-700/30 px-4 text-sm text-slate-500 opacity-60 cursor-not-allowed"
                />
              </div>
              <div className="flex flex-col gap-2">
                <label className="text-[10px] font-black uppercase text-slate-500 tracking-widest">
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="h-11 rounded-lg bg-[#0f172a]/30 border border-slate-700/30 px-4 flex items-center gap-3 text-sm text-slate-400 opacity-80 cursor-not-allowed">
                <div className="w-5 h-5 rounded-full bg-indigo-500/20 text-indigo-400 flex items-center justify-center text-[10px] font-black">
                  ID
                </div>
