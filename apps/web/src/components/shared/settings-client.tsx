  ...
  useEffect(() => {
    async function loadSession() {
      ...
      if (session?.user) {
        ...
        const userMetadata = session.user.user_metadata as { full_name?: string; role?: string } | undefined;
        setUserName(userMetadata?.full_name || session.user.email || "");
    ...
    <div className="flex flex-col w-full max-w-[900px] gap-8 mx-auto pb-20 mt-[-10px] text-slate-300">
      ...
      <div className="flex items-center gap-4 p-5 rounded-2xl bg-[#1e293b] border border-[#2b3952] shadow-sm">
         ...
         <div className="flex flex-col">
            ...
            <span className="text-sm font-medium text-slate-400">{email || "No email"} · <span className="text-indigo-400">{role || "No role"}</span></span>
         </div>
      </div>
           ...
           <div className="flex flex-col gap-5 text-slate-500">
              <div className="grid grid-cols-2 gap-4">
                 <div className="flex flex-col gap-2">
                   ...
                   <input disabled type="text" value={email || "Not available"} className="h-11 rounded-lg bg-[#0f172a]/30 border border-slate-700/30 px-4 text-sm text-slate-500 opacity-60 cursor-not-allowed" />
                 </div>
                 <div className="flex flex-col gap-2">
              ...
              <div className="flex flex-col gap-2">
                 ...
                 <input disabled type="text" value="Not available" className="h-11 rounded-lg bg-[#0f172a]/30 border border-slate-700/30 px-4 text-sm text-slate-500 opacity-60 cursor-not-allowed" />
              </div>
           </div>
        </div>
