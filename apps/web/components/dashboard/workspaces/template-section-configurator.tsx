...
export function TemplateSectionConfigurator({
  sections,
  onChange,
}: TemplateSectionConfiguratorProps) {
  const addSection = () => {
    onChange([
      ...sections,
      {
  ...
  return (
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <h3 className="text-sm font-semibold text-textContrast">
          Sections & Blueprints
        </h3>
        <Button
          type="button"
      ...
      {sections.length === 0 && (
        <div className="p-8 border-2 border-dashed border-borderSubtle rounded-xl text-center">
          <p className="text-sm text-textMuted text-center">
            No sections defined. Add one to start building your blueprint.
          </p>
        </div>
      )}
      ...
      <div className="flex flex-col gap-3">
        {sections.map((section, idx) => (
          <Card
            key={idx}
            className="relative group overflow-visible border-brandAccent/20"
          >
            <div className="absolute -top-3 -left-3">
              <Badge
                variant="brand"
                className="shadow-lg border-2 border-backgroundMain"
              >
            ...
            <div className="flex flex-col gap-4 pt-2">
              ...
              <div className="border-t border-borderSubtle pt-4">
                <span className="text-[10px] uppercase tracking-wider font-bold text-textMuted mb-2 block">
                  Pedagogical Controls (Optional)
                </span>
