...
import { Building2, Users, FileCheck, BarChart3, ChevronDown } from "lucide-react";
...
import { FacultyActivityTable } from "./FacultyActivityTable";
import type { InstitutionDashboardSummaryResponse, PaperRecord } from "../../../lib/dashboard";
...
export function DepartmentOverviewContent({
    ...
    async function loadData() {
      try {
        const [summaryResponse, papersResponse] = await Promise.all([
          apiRequest<InstitutionDashboardSummaryResponse>("/dashboard/summary", {
            method: "GET",
            accessToken,
            institutionId
          }),
          apiRequest<PaperRecord[]>("/content/papers", {
            method: "GET",
            accessToken,
            institutionId
          })
        ...
        if (isMounted) {
          setStatus(error instanceof Error ? error.message : "Failed to load department data");
        }
      } finally {
  ...
  const stats = useMemo<DepartmentStats>(() => ({
    ...
    pendingApprovals: papers.filter(p => ["submitted", "in_review"].includes(p.status)).length,
    ...
    papers.forEach(paper => {
      ...
      paper.sections?.forEach(section => {
        section.questions?.forEach(q => {
          ...
          else if (q.difficulty === "medium") coverage[subject].difficulty.medium++;
  ...
  const facultyActivity = useMemo<FacultyActivity[]>(() => {
    ...
    summary?.recentQuestions?.forEach(q => {
      ...
      if (!faculty[name]) {
        faculty[name] = { name, questionsAdded: 0, drafts: 0, submitted: 0, lastActive: null };
      }
    ...
    <div className="flex flex-col gap-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="flex flex-col gap-1">
          ...
          <h1 className="text-4xl font-black tracking-tighter text-white">Department Overview</h1>
          <p className="text-zinc-500 font-medium max-w-xl">Monitor question coverage and faculty activity</p>
        </div>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-black tracking-tight text-white">Subject Coverage</h2>
          <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">{subjectCoverage.length} Subjects</span>
        </div>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-black tracking-tight text-white">Faculty Activity</h2>
          <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">{facultyActivity.length} Members</span>
        </div>
      ...
      <div className="flex flex-col gap-6">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-black tracking-tight text-white">Pending Approvals</h2>
          <span className="text-xs font-bold text-zinc-500 uppercase tracking-widest">{pendingApprovals.length} Papers</span>
        </div>

        {pendingApprovals.length === 0 ? (
          <Card className="!bg-white/5 border-white/5 border-dashed !rounded-2xl p-12 text-center">
            <p className="text-zinc-500 font-bold">No papers pending approval</p>
          </Card>
        ) : (
            ...
            columns={[
              {
                key: "title",
                label: "Paper Title",
                render: (value) => <span className="font-bold text-white line-clamp-1">{value}</span>
              },
              {
                key: "faculty",
                label: "Faculty",
                render: (value) => <span className="text-zinc-400">{value}</span>
              },
              {
                key: "subject",
                label: "Subject",
              ...
              },
              {
                key: "submitted",
                label: "Submitted",
                render: (value) => <span className="text-xs text-zinc-500">{value}</span>
              },
              {
                key: "status",
                label: "Status",
                render: (value) => (
                  <span className={`text-xs font-bold uppercase tracking-wide px-2 py-1 rounded ${
                    value === "submitted" ? "bg-amber-500/10 text-amber-400" : "bg-indigo-500/10 text-indigo-400"
                  }`}>
                    {value}
                  </span>
                )
