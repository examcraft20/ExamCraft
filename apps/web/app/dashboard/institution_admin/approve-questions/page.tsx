...
import { CheckSquare, CheckCircle, XCircle, RefreshCw, MessageSquare, ChevronDown } from "lucide-react";
...
export default function ApproveQuestionsPage() {
  ...
  useEffect(() => {
    ...
    apiRequest<Question[]>("/content/questions", { method: "GET", accessToken, institutionId })
      .then((data) => { if (mounted) setQuestions(Array.isArray(data) ? data : []); })
  ...
  const handleReview = async (questionId: string, action: "approve" | "reject") => {
    ...
    try {
      await apiRequest<Question>(`/content/questions/${questionId}/review`, {
        ...
        accessToken,
        institutionId,
        body: JSON.stringify({ action, comment: reviewComments[questionId] || "" }),
      });
      refresh();
    } catch (e) {
      console.error(e);
  ...
  const statusMap = { pending: ["draft", "pending"], approved: ["ready", "approved"], rejected: ["rejected"] };
  const allSubjects = [...new Set(questions.map((q) => q.subject).filter(Boolean))];
  const allDepts = [...new Set(questions.map((q) => q.departmentId).filter(Boolean))] as string[];

  const filtered = useMemo(() => {
    const statuses = statusMap[activeTab];
    return questions.filter((q) => {
      const matchStatus = statuses.some((s) => q.status?.toLowerCase().includes(s));
  ...
  const counts = {
    pending: questions.filter((q) => statusMap.pending.some((s) => q.status?.toLowerCase().includes(s))).length,
    approved: questions.filter((q) => statusMap.approved.some((s) => q.status?.toLowerCase().includes(s))).length,
    rejected: questions.filter((q) => q.status?.toLowerCase().includes("rejected")).length,
  };

  const diffColor = (d: string) =>
    d === "hard" || d === "difficult" ? "bg-red-500/20 text-red-400" :
  ...
  return (
    <div className="flex flex-col gap-8 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-3xl font-black tracking-tight text-white">Approve Questions</h1>
          </div>
          <p className="text-[#8b9bb4] text-sm font-medium mt-1">Review faculty submissions before they enter the question bank.</p>
        </div>
        <button
          onClick={refresh}
      ...
      <div className="flex flex-wrap gap-3">
        ...
        <div className="ml-auto text-sm text-[#8b9bb4] self-center font-medium">{filtered.length} questions</div>
      </div>
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center p-20 gap-4">
            ...
            <p className="text-[#8b9bb4] font-medium">
              {activeTab === "pending" ? "All caught up! No pending questions." : `No ${activeTab} questions found.`}
            </p>
          </div>
        ) : (
          <div className="divide-y divide-white/5">
            {filtered.map((q) => (
              <div key={q.id} className="p-5 hover:bg-white/[0.02] transition-colors">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    <p className="text-white font-semibold text-sm leading-snug line-clamp-2 mb-2">{q.title}</p>
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="text-[11px] font-bold text-indigo-400 bg-indigo-500/10 px-2.5 py-1 rounded-full">{q.subject}</span>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded capitalize ${diffColor(q.difficulty)}`}>{q.difficulty}</span>
                      {q.bloomLevel && <span className="text-[10px] text-[#8b9bb4] font-medium">{q.bloomLevel}</span>}
                      {q.unitNumber && <span className="text-[10px] text-[#8b9bb4]">Unit {q.unitNumber}</span>}
                    </div>
                  </div>
                  ...
                  {activeTab === "pending" && (
                    <div className="flex items-center gap-2 shrink-0">
                      ...
                      >
                        {actionLoadingId === q.id ? <Spinner size="sm" /> : <XCircle size={14} />} Reject
                      </button>
                      <button
                        disabled={actionLoadingId === q.id}
                      ...
                      >
                        {actionLoadingId === q.id ? <Spinner size="sm" /> : <CheckCircle size={14} />} Approve
                      </button>
                    </div>
                  )}
                  {activeTab !== "pending" && (
                    <span className={`text-[11px] font-bold px-3 py-1 rounded-full shrink-0 ${
                      activeTab === "approved" ? "bg-green-500/10 text-green-400" : "bg-red-500/10 text-red-400"
                    } capitalize`}>
                      {q.status}
                    </span>
                  )}
                ...
                {q.reviewComment && (
                  <p className="mt-2 text-xs text-[#8b9bb4] italic">"{q.reviewComment}"</p>
                )}
              </div>
            ))}
          </div>
