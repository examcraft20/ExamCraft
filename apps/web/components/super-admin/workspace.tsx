...
export function SuperAdminWorkspace({ accessToken }: SuperAdminWorkspaceProps) {
  const [summary, setSummary] =
    useState<PlatformDashboardSummaryResponse | null>(null);
  ...
  useEffect(() => {
    ...
    async function loadWorkspace() {
      try {
        const [summaryResponse, institutionsResponse, auditResponse] =
          await Promise.all([
            apiRequest<PlatformDashboardSummaryResponse>(
              "/institution/platform-summary",
              { method: "GET", accessToken },
            ),
            apiRequest<PlatformInstitutionRecord[]>(
              "/institution/platform-institutions",
              { method: "GET", accessToken },
            ),
            apiRequest<PlatformAuditEvent[]>("/institution/platform-audit-feed", {
              method: "GET",
              accessToken,
            }),
          ]);
        ...
        if (isMounted) {
          ...
          setStatusDrafts(
            Object.fromEntries(
              institutionsResponse.map((ins) => [ins.id, ins.status]),
            ),
          );
        }
      } catch (error) {
        if (isMounted)
          setStatus(
            error instanceof Error
              ? error.message
              : "Unable to synchronize platform control.",
          );
      } finally {
  ...
  const filteredInstitutions = useMemo(() => {
    ...
    return institutions.filter((ins) =>
      [ins.name, ins.slug, ins.institutionType, ins.status]
        .join(" ")
        .toLowerCase()
        .includes(needle),
    );
  ...
  async function updateInstitutionStatus(institutionId: string) {
    ...
    try {
      const updatedInstitution = await apiRequest<PlatformInstitutionRecord>(
        `/institution/platform-institutions/${institutionId}/status`,
        {
          method: "PATCH",
          accessToken,
          body: JSON.stringify({
      ...
      setInstitutions((current) =>
        current.map((ins) =>
          ins.id === institutionId
            ? { ...ins, status: updatedInstitution.status }
            : ins,
        ),
      );
      setStatus(
        `Institutional lifecycle updated for "${updatedInstitution.name}".`,
      );
      const refreshedAudit = await apiRequest<PlatformAuditEvent[]>(
        "/institution/platform-audit-feed",
        { method: "GET", accessToken },
      );
    ...
    <div className="flex flex-col gap-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[
          {
            label: "Global Shards",
            value: summary?.totals.institutions ?? 0,
            icon: Building2,
            color: "text-blue-400",
            sub: "Active Tenants",
          },
          ...
          },
          {
            ...
            value: summary?.totals.pendingInvitations ?? 0,
            icon: Mail,
          ...
          },
          {
            label: "Content Flux",
            value:
              (summary?.totals.questions ?? 0) +
              (summary?.totals.templates ?? 0),
            icon: Database,
            color: "text-indigo-400",
            sub: "Pedagogical Assets",
          },
        ].map((stat, i) => (
          <Card
            key={i}
            className="!bg-zinc-900 border-white/5 !rounded-3xl p-8 flex items-start gap-6 shadow-2xl relative overflow-hidden group hover:border-white/10 transition-all"
          >
            <div className="absolute top-0 right-0 w-24 h-24 bg-white/5 blur-2xl group-hover:bg-white/10 transition-all" />
            <div
              className={`w-14 h-14 rounded-2xl bg-white/5 border border-white/5 flex items-center justify-center ${stat.color}`}
            >
            ...
            <div className="flex flex-col">
              ...
              <span className="text-[10px] font-bold text-zinc-600 mt-2 uppercase tracking-widest">
                {stat.sub}
              </span>
            </div>
          </Card>
          ...
          <div className="flex items-center justify-between gap-4">
            <div className="flex flex-col gap-1">
              ...
              <p className="text-zinc-500 text-sm font-medium">
                Coordinate lifecycle states for all platform shards.
              </p>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="p-32 rounded-[2.5rem] bg-white/5 border border-white/5 border-dashed text-center flex flex-col items-center gap-6">
                ...
                <p className="text-zinc-500 font-bold tracking-tight">
                  Zero matching tenancies identified.
                </p>
              </div>
            ) : (
              filteredInstitutions.map((ins) => (
                <div
                  key={ins.id}
                  className="group p-8 rounded-[2.5rem] bg-zinc-900 border border-white/5 hover:border-white/10 transition-all flex flex-col gap-8 shadow-xl relative overflow-hidden"
                >
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-5">
                      ...
                      <div className="flex flex-col gap-0.5">
                        <h4 className="text-xl font-black tracking-tight text-white group-hover:text-indigo-300 transition-colors uppercase">
                          {ins.name}
                        </h4>
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-4 p-6 rounded-2xl bg-white/5 border border-white/5">
                    {[
                      {
                        label: "Auth Users",
                        value: ins.usage.activeUsers,
                        icon: Users,
                      },
                      {
                        label: "Sync Requests",
                        value: ins.usage.pendingInvitations,
                        icon: Mail,
                      },
                      {
                        label: "Question Assets",
                        value: ins.usage.questions,
                        icon: Database,
                      },
                      {
                        label: "Templates",
                        value: ins.usage.templates,
                        icon: LayoutGrid,
                      },
                    ].map((cube, i) => (
        ...
        <div className="flex flex-col gap-8 sticky top-6">
          <div className="flex flex-col gap-1">
            ...
            <p className="text-zinc-500 text-[10px] font-black uppercase tracking-[0.2em]">
              Live Cross-Tenancy Activity Audit
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4 overflow-y-auto max-h-[800px] pr-2 custom-scrollbar">
            {isLoading ? (
              ...
              <p className="text-zinc-600 text-sm font-bold text-center py-20 italic">
                No global activity synchronized.
              </p>
            ) : (
              auditEvents.map((event) => (
                <div
                  key={event.id}
                  className="p-5 rounded-2xl bg-zinc-900 border border-white/5 hover:border-white/10 transition-all flex gap-4 relative overflow-hidden group"
                >
                  ...
                  <div className="flex flex-col gap-0.5">
                    <h5 className="text-sm font-black text-white group-hover:text-indigo-200 transition-colors">
                      {event.title}
                    </h5>
                    <div className="flex items-center gap-3">
          ...
          <Button
            variant="secondary"
            fullWidth
            className="py-4 border-white/5 rounded-xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 opacity-60 hover:opacity-100 transition-opacity"
          >
