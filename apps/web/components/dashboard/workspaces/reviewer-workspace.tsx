...
export function ReviewerWorkspace({
  ...
  const reviewPapers = useMemo(
    () => papers.filter((p) => ["submitted", "in_review"].includes(p.status)),
    [papers],
  );
  ...
  const reviewQueue = useMemo(() => {
    ...
    const filteredByStatus = items.filter(
      (item) =>
        ["submitted", "draft", "in_review"].includes(item.status) ||
        item.reviewComment,
    );
  ...
  const onHandleReview = (
    item: (typeof reviewQueue)[number],
      ...
      <div className="grid md:grid-cols-4 gap-6">
        {[
          ...
          },
          {
            ...
            value: questions.filter((q) => q.status === "ready").length,
            icon: Database,
            color: "text-emerald-400",
          },
          {
            label: "Templates Released",
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          <div className="flex flex-col gap-1">
            <h2 className="text-3xl font-black tracking-tighter text-white flex items-center gap-3">
              ...
              <div className="px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-[9px] font-black uppercase tracking-widest text-indigo-400">
                Live Feedback
              </div>
            </h2>
            <p className="text-zinc-500 text-sm font-medium">
              Verify pedagogical integrity and release approved examination
              assets.
            </p>
          </div>
            ...
            <div className="p-32 rounded-[2.5rem] bg-white/5 border border-white/5 border-dashed text-center flex flex-col items-center gap-6">
              ...
              <div className="flex flex-col gap-1">
                <h4 className="text-xl font-bold text-zinc-600 tracking-tight">
                  Assurance queue clear.
                </h4>
                <p className="text-zinc-500 text-sm font-medium">
                  All pending institutional assets have been synchronized and
            ...
            reviewQueue.map((item) => (
              ...
              >
                ...
                <div className="flex items-center justify-between gap-6">
                  ...
                  <div
                    className={`px-4 py-1.5 rounded-xl border text-[10px] font-black uppercase tracking-widest transition-all ${item.status === "submitted" || item.status === "in_review" ? "bg-amber-500/10 border-amber-500/20 text-amber-400 animate-pulse" : "bg-white/5 border-white/10 text-zinc-600"}`}
                  >
                    {item.status}
                  </div>
                </div>
                ...
                <div className="flex flex-col md:flex-row items-center gap-6">
                  ...
                  <div className="flex items-center gap-3 w-full md:w-fit">
                    ...
                    <Button
                      ...
                      loading={
                        activeActionKey === `${item.kind}:${item.id}:approve`
                      }
                    ...
                    >
                      {!activeActionKey && <CheckCircle2 size={16} />} Approve
                      Release
                    </Button>
                    <Button
                      variant="secondary"
                      ...
                      loading={
                        activeActionKey === `${item.kind}:${item.id}:reject`
                      }
