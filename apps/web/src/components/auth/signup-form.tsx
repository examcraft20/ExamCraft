...
export function SignupForm() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (role === 'Reviewer') roleKey = 'reviewer_approver';
      
      const response = await fetch(`${env.NEXT_PUBLIC_API_URL}/v1/auth/signup`, {
        method: "POST",
      ...
      if (!response.ok || result.error) {
        setFieldErrors({ form: result.error?.message || "Failed to create account." });
        setIsLoading(false);
        return;
      }
      ...
      setFieldErrors({ form: "An unexpected error occurred. Please try again." });
      setIsLoading(false);
    }
  }
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <div className="relative">
            <input
              ...
              placeholder="••••••••"
              className={`${INPUT_FIELD_CLASSES} ${fieldErrors.password ? 'border-red-500/50 focus:border-red-500 focus:ring-red-500/20' : ''} pr-10`}
            />
            <button
              type="button"
              disabled={isLoading}
        ...
        <div>
          ...
          <input
            ...
            placeholder="••••••••"
            className={`${INPUT_FIELD_CLASSES} ${fieldErrors.confirmPassword ? 'border-red-500/50 focus:border-red-500 focus:ring-red-500/20' : ''}`}
          />
      ...
      {fieldErrors.form && (
        <p className="text-red-400 text-sm mt-1 text-center font-medium">{fieldErrors.form}</p>
      )}
