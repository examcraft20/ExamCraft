...
export function AcademicHeadWorkspace({
  ...
  const filteredQuestions = useMemo(() => {
    ...
    return questions.filter((q) =>
      [q.title, q.subject, q.bloomLevel, q.difficulty]
        .join(" ")
        .toLowerCase()
        .includes(needle),
    );
  ...
  const filteredTemplates = useMemo(() => {
    ...
    return templates.filter((t) =>
      [t.name, t.examType, t.status].join(" ").toLowerCase().includes(needle),
    );
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                  <div className="flex flex-col gap-1">
                    <h2 className="text-3xl font-black tracking-tighter text-white uppercase">
                      Pedagogical Review Desk
                    </h2>
                    <p className="text-zinc-500 text-sm font-medium">
                      Verify Bloom&apos;s alignment and difficulty calibration
                      for new submissions.
                    </p>
                  </div>
                ...
                <div className="grid gap-6">
                  {isLoading ? (
                    ...
                    filteredQuestions.map((q) => (
                      <div
                        key={q.id}
                        className="group p-8 rounded-[2.5rem] bg-zinc-950 border border-white/5 hover:border-white/10 transition-all flex flex-col gap-8 shadow-xl relative overflow-hidden"
                      >
                        <div className="flex items-center justify-between">
                          <div className="flex flex-col gap-1">
                            <h4 className="text-xl font-black tracking-tight text-white group-hover:text-amber-300 transition-colors uppercase">
                              {q.title}
                            </h4>
                            <div className="flex items-center gap-3">
                              <span className="px-2 py-0.5 rounded bg-amber-500/10 text-amber-500 text-[8px] font-black uppercase tracking-widest">
                                {q.bloomLevel}
                              </span>
                              <span className="px-2 py-0.5 rounded bg-zinc-800 text-zinc-400 text-[8px] font-black uppercase tracking-widest">
                                {q.difficulty}
                              </span>
                          ...
                          <div
                            className={`px-4 py-1.5 rounded-xl border text-[10px] font-black uppercase tracking-widest ${q.status === "submitted" ? "bg-amber-500/10 border-amber-500/20 text-amber-400" : "bg-white/5 border-white/10 text-zinc-600"}`}
                          >
                            {q.status}
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col md:flex-row items-center gap-6">
                          ...
                          <div className="flex items-center gap-3 w-full md:w-fit">
                            <Button
                              onClick={() =>
                                onHandleReview("question", q.id, "approve")
                              }
                              loading={
                                activeActionKey === `question:${q.id}:approve`
                              }
                            ...
                            <Button
                              variant="secondary"
                              onClick={() =>
                                onHandleReview("question", q.id, "reject")
                              }
                              loading={
                                activeActionKey === `question:${q.id}:reject`
                              }
              ...
              <Card className="!bg-zinc-900 border-white/5 !rounded-[2.5rem] p-10 shadow-2xl relative overflow-hidden">
                ...
                <div className="flex items-center gap-4 mb-10">
                  ...
                  <div className="flex flex-col">
                    ...
                    <p className="text-zinc-500 text-sm font-medium mt-1">
                      Review paper structures before global institution release.
                    </p>
                  </div>
                </div>
                ...
                <div className="grid gap-6">
                  {isLoading ? (
                    ...
                    <p className="text-zinc-600 text-sm font-bold text-center py-20 italic">
                      No templates awaiting validation.
                    </p>
                  ) : (
                    filteredTemplates.map((t) => (
                      <div
                        key={t.id}
                        className="p-6 rounded-3xl bg-zinc-950 border border-white/5 hover:border-white/10 transition-all flex items-center justify-between"
                      >
                        ...
                        <div className="flex items-center gap-3">
                          <Button
                            size="sm"
                            onClick={() =>
                              onHandleReview("template", t.id, "approve")
                            }
                            loading={
                              activeActionKey === `template:${t.id}:approve`
                            }
