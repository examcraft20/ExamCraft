...
import { Card, Button, Spinner } from "@examcraft/ui";
...
export function PaperReviewPanel({
  ...
  useEffect(() => {
    ...
    async function loadPaper() {
      try {
        ...
        const result = await apiRequest<PaperPayload>(`/papers/${paperId}`, {
          method: "GET",
          accessToken: session.access_token,
          institutionId,
        });

        if (isMounted) setPaper(result);
      } catch {
  ...
  const handleReviewAction = async (action: "approve" | "reject") => {
    ...
    try {
      ...
      router.push(
        `/dashboard/reviewer_approver/review?institutionId=${institutionId}`
      );
      router.refresh();
    } catch (submitError) {
      setError(
    ...
    <div className="flex flex-col gap-8 max-w-5xl mx-auto w-full pb-20">
      ...
      <div className="grid lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 flex flex-col gap-8">
          {paper.metadata.sections.map((section, sectionIndex) => (
            <div key={sectionIndex} className="flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-3">
                {section.questions.map((question, questionIndex) => (
                  <Card
                    key={question.id}
                    className="!bg-zinc-900/50 border-white/5 !p-4 flex flex-col gap-3"
                  >
              ...
              <div className="flex flex-col gap-3 mt-4">
                ...
                >
                  {isSubmitting === "approve" ? (
                    ...
                    <>
                      <CheckCircle2 size={16} /> Approve and Sign Off
                    </>
                  )}
                </Button>
                ...
                >
                  {isSubmitting === "reject" ? (
                    ...
                    <>
                      <XCircle size={16} /> Reject and Return to Faculty
                    </>
                  )}
                </Button>
              </div>
