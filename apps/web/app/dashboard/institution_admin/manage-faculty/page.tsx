...
import { Button, Card, Spinner, StatusMessage, formatRoleName } from "@examcraft/ui";
            ...
            <tbody className="divide-y divide-white/5">
              {filteredUsers.map((user) => (
                <tr key={user.institutionUserId} className="hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="px-8 py-5">
                    <div className="flex flex-wrap gap-1">
                      {user.roleCodes.map(rc => (
                        <span key={rc} className="px-2 py-0.5 rounded bg-white/5 border border-white/5 text-[9px] font-black uppercase tracking-tighter text-zinc-400">
                          {rc.replace('_', ' ')}
                        </span>
                      ))}
                    </div>
                  ...
                  <td className="px-8 py-5">
                    <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${user.status === 'active' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-amber-500/10 border-amber-500/20 text-amber-400'}`}>
                      {user.status}
                    </span>
                  </td>
