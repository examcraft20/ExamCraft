...
type PaperReviewStatus = "draft" | "submitted" | "in_review" | "approved" | "rejected" | "published" | "archived";
...
export class ContentService {
  ...
  async listQuestions(tenantContext: TenantContext) {
    const { data, error } = await this.supabaseAdminClient
      ...
      .select("id, title, subject, bloom_level, difficulty, tags, course_outcomes, unit_number, department_id, course_id, status, metadata, created_at")
  ...
  async getQuestion(
    ...
    const existing = await this.loadQuestionForInstitution(questionId, tenantContext.institutionId);
    return {
      id: existing.id,
  ...
  async editQuestion(
    ...
    if (payload.bloomLevel !== undefined) updates.bloom_level = payload.bloomLevel;
    if (payload.difficulty !== undefined) updates.difficulty = payload.difficulty;
    if (payload.tags !== undefined) updates.tags = payload.tags;
    if (payload.courseOutcomes !== undefined) updates.course_outcomes = payload.courseOutcomes;
    if (payload.unitNumber !== undefined) updates.unit_number = payload.unitNumber;
    if (payload.departmentId !== undefined) updates.department_id = payload.departmentId;
    ...
    if (error || !data) {
      throw new InternalServerErrorException("Unable to update institution question.");
    }
  ...
  async archiveQuestion(
    ...
    if (error) {
      throw new InternalServerErrorException("Unable to archive institution question.");
    }
  }
  ...
  async createBulkQuestions(
    ...
    if (error || !data) {
      throw new InternalServerErrorException("Unable to create institution questions.");
    }
  ...
  async generatePaper(
    ...
    const template = await this.loadTemplateForInstitution(payload.templateId, tenantContext.institutionId);
    ...
    for (const section of sections) {
      const { data: questions, error } = await this.supabaseAdminClient.rpc(
        "get_random_questions",
        {
          ...
          p_difficulties: section.allowedDifficulty?.length > 0 ? section.allowedDifficulty : null,
          p_bloom_levels: section.allowedBloomLevels?.length > 0 ? section.allowedBloomLevels : null,
          p_unit_numbers: section.allowedUnitNumbers?.length > 0 ? section.allowedUnitNumbers : null,
    ...
    if (paperError) {
      throw new InternalServerErrorException("Failed to save the generated paper.");
    }
  ...
  async listPapers(tenantContext: TenantContext) {
    ...
    if (error || !data) {
      throw new InternalServerErrorException("Unable to load institution papers.");
    }
  ...
  async submitPaper(tenantContext: TenantContext, currentUser: AuthenticatedUser, paperId: string) {
    const { data, error } = await this.supabaseAdminClient
      .from("institution_papers")
      .update({
        status: "submitted",
    ...
    if (error || !data) {
      throw new InternalServerErrorException("Unable to submit paper for review.");
    }
    ...
    try {
      ...
      if (roles) {
        ...
        if (reviewers && reviewers.length > 0) {
          const emails = reviewers.map((r: any) => r.institution_users?.users?.email).filter(Boolean);
          for (const email of emails) {
            this.mailerService.sendPaperSubmittedForReview(email, data.title, currentUser.email || 'A faculty member').catch(e => this.logger.error('Failed to send paper submitted email', e));
          }
        }
      }
    } catch (e) {
  ...
  async reviewPaper(
    ...
    const { data: existingPaper, error: loadError } = await this.supabaseAdminClient
      .from("institution_papers")
    ...
    const nextMetadata = this.buildNextMetadata(existingPaper.metadata, currentUser, payload);
    const nextStatus = this.resolvePaperStatus(existingPaper.status as PaperReviewStatus, payload.action);
    ...
    if (error || !data) {
      throw new InternalServerErrorException("Unable to update paper review state.");
    }
    ...
    try {
      if (payload.action === 'approve' || payload.action === 'reject') {
        ...
        if (submitterUser?.email) {
          this.mailerService.sendPaperReviewed(
            submitterUser.email, 
            existingPaper.title, 
            payload.action === 'approve' ? 'approved' : 'rejected', 
            payload.comment || ''
          ).catch(e => this.logger.error('Failed to send paper reviewed email', e));
        }
      }
    } catch (e) {
  ...
  async analyzeSyllabusAndGenerate(
    ...
    try {
      ...
      generatedQuestions = items.slice(0, count).map((item: any) => ({
        ...
        bloomLevel: ["Remember", "Understand", "Apply", "Analyze", "Evaluate", "Create"].includes(item.bloom_level)
          ? item.bloom_level
          : "Understand",
