...
import { Button, Card, Input, Select, Spinner, StatusMessage } from "@examcraft/ui";
...
export function FacultyWorkspace({ accessToken, institutionId, institutionName }: FacultyWorkspaceProps) {
  const [activeTab, setActiveTab] = useState<"questions" | "templates" | "papers" | "global">("questions");
  ...
  const [sections, setSections] = useState<any[]>([
    { title: "Section A", questionCount: 5, marks: 50, allowedDifficulty: ["easy", "medium"] }
  ]);
  ...
  useEffect(() => {
    ...
    async function loadContent() {
      try {
        const [templateResponse, deptsRes, coursesRes, subsRes] = await Promise.all([
        ...
        if (isMounted) {
          setTemplateStatus(error instanceof Error ? error.message : "Unable to load faculty templates.");
        }
  ...
  async function onGenerate() {
    ...
    try {
      ...
      setTemplateStatus('Paper generation is not yet implemented in demo mode.');
    } catch (error) {
      setTemplateStatus(error instanceof Error ? error.message : 'Failed to generate paper.');
    } finally {
  ...
  const filteredCourses = useMemo(() => {
    return templateForm.departmentId ? courses.filter(c => c.department_id === templateForm.departmentId) : [];
  }, [courses, templateForm.departmentId]);

  const filteredSubjects = useMemo(() => {
    return templateForm.courseId ? subjects.filter(s => s.course_id === templateForm.courseId) : [];
  ...
  async function handleGenerate(templateId: string, templateName: string) {
    ...
    try {
      ...
      setTemplateStatus(`Draft paper generated from "${templateName}". Check the Papers tab!`);
      setActiveTab("papers");
    } catch (error) {
      setTemplateStatus(error instanceof Error ? error.message : "Generation failed.");
    } finally {
      setIsGenerating(null);
    }
  }
  ...
  return (
    <div className="flex flex-col gap-10">
      ...
      <div className="relative">
        ...
        {activeTab === "papers" && (
          <PaperWorkspace accessToken={accessToken} institutionId={institutionId} institutionName={institutionName} />
        )}
          ...
          <div className="grid lg:grid-cols-[1fr_1.5fr] gap-10 items-start">
            ...
            <Card className="!bg-zinc-900 border-white/5 !rounded-[2.5rem] p-8 shadow-2xl relative overflow-hidden group">
              ...
              <div className="flex items-center gap-3 mb-8">
                 ...
                 <h2 className="text-2xl font-black tracking-tight text-white">Create Template</h2>
              </div>
              ...
              <form className="flex flex-col gap-6" onSubmit={createTemplate}>
                <div className="grid grid-cols-1 gap-4">
                  <Select
                    ...
                    onChange={e => setTemplateForm({ ...templateForm, departmentId: e.target.value, courseId: "", subjectId: "" })}
                    options={[
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Select
                      ...
                      onChange={e => setTemplateForm({ ...templateForm, courseId: e.target.value, subjectId: "" })}
                      options={[
                ...
                <Button type="submit" fullWidth className="bg-white text-black py-6 rounded-2xl font-black text-lg hover:scale-[1.02] active:scale-95 transition-all shadow-xl">
                  Deploy Template
                </Button>
              </form>
            ...
            <div className="flex flex-col gap-6">
               <div className="flex items-center justify-between gap-4">
                  <div className="flex flex-col gap-1">
                     <h2 className="text-2xl font-black tracking-tight text-white">Institutional Library</h2>
                     <p className="text-zinc-500 text-sm font-medium">Standardized paper templates for rapid pedagogical deployment.</p>
                  </div>
               ...
               <div className="grid gap-4 mt-2">
                 {isLoading ? (
                    ...
                    <div className="p-16 rounded-[2.5rem] bg-white/5 border border-white/5 border-dashed text-center flex flex-col items-center gap-4">
                       ...
                       <p className="text-zinc-500 font-bold tracking-tight">No institutional templates identified.</p>
                    </div>
                 ) : (
                    filteredTemplates.map((template) => (
                       <TemplateCard 
                         ...
                         onGenerate={() => handleGenerate(template.id, template.name)}
                       />
                    ))
                 )}
               </div>
        ...
        {activeTab === "global" && (
          <GlobalTemplateLibrary 
            ...
            onCloned={(newT) => {
              ...
              setTemplateStatus(`Global template "${newT.name}" imported to your institution.`);
            }} 
          />
        )}
      </div>
...
function TemplateCard({ template, onGenerate, isGenerating }: { template: Template; onGenerate: () => void; isGenerating: boolean }) {
  return (
    <div className="p-6 rounded-[2rem] bg-zinc-900 border border-white/5 hover:border-white/10 transition-all group flex items-center justify-between gap-6 shadow-lg overflow-hidden relative">
       ...
       <div className="flex items-center gap-5">
          ...
          <div className="flex flex-col gap-0.5">
             <h4 className="text-lg font-black tracking-tight text-white group-hover:text-indigo-300 transition-colors">{template.name}</h4>
             <div className="flex items-center gap-3">
                <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">{template.examType}</span>
                <div className="w-1 h-1 rounded-full bg-zinc-700" />
                <span className="text-[10px] font-black uppercase tracking-widest text-zinc-500">{template.totalMarks} Marks</span>
                <div className="w-1 h-1 rounded-full bg-zinc-700" />
                <span className={`text-[10px] font-black uppercase tracking-widest ${template.status === "ACTIVE" ? "text-emerald-400" : "text-amber-400"}`}>{template.status}</span>
             </div>
          </div>
       </div>
