...
export default function SettingsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        try {
          const res = await apiRequest<any>("/institution/context", { method: "GET", accessToken: accessToken!, institutionId: institutionId! });
          if (mounted && res?.institutionContext) {
            const partial = {
              ...defaultSettings,
  ...
  const handleSave = async () => {
    ...
    setStatus(null);
    try {
      await apiRequest("/institution/branding", {
        method: "PATCH",
        accessToken,
        institutionId,
        body: JSON.stringify({
