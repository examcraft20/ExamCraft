...
export function useAdminContext() {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      // If we still don't have an institutionId after session is loaded, 
      // redirect to dashboard home to select one
      if (!instId) {
          console.warn('useAdminContext: No institution ID found, redirecting to dashboard');
