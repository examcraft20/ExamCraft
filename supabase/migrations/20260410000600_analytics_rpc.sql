...
BEGIN
  ...
  -- 6. Faculty count (only users with 'faculty' role)
  SELECT count(distinct iu.id) INTO v_faculty_count 
  FROM institution_users iu
  JOIN institution_user_roles iur ON iu.id = iur.institution_user_id
  JOIN roles r ON iur.role_id = r.id
  WHERE iu.institution_id = p_institution_id
    AND r.code = 'faculty'
