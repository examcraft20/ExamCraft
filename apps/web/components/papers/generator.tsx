    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Failed to generate paper. Re-check template tolerances.",
      );
        ...
        <div className="p-12 rounded-2xl bg-zinc-900/30 border border-white/5 border-dashed flex flex-col items-center justify-center gap-4 text-center">
          ...
          <div className="max-w-xs">
            ...
            <p className="text-slate-400 text-sm leading-relaxed">
              You need at least one &apos;Published&apos; template before you
              can auto-generate a paper. Go back to Templates and build or
              publish one.
            </p>
          </div>
          <Link
                ...
                {templates.map((t) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <h4
                        className={`font-bold transition-colors ${isSelected ? "text-indigo-300" : "text-white"}`}
                      >
                        {t.name}
                      </h4>
                      <p className="text-xs font-medium text-zinc-500 mt-1">
                        {t.examType} • {t.durationMinutes} mins • {t.totalMarks}{" "}
                        marks
                      </p>
                    </button>
                  );
              ...
              {activeTemplate ? (
                <div className="flex flex-col gap-6">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="p-3 rounded-lg bg-black/40 border border-white/5 flex flex-col">
                      ...
                      <span className="text-sm font-bold text-white mt-1">
                        {activeTemplate.totalMarks} Marks
                      </span>
                    </div>
                  </div>

                  <p className="text-sm text-slate-400 leading-relaxed font-medium">
                    ExamCraft will now query the Question Bank, select items
                    matching your Blueprint rules (difficulty, taxonomy,
                    scopes), and assemble a finalized exam randomly.
                  </p>
                ...
                <div className="py-12 flex flex-col items-center justify-center text-center gap-3">
                  ...
                  <p className="text-sm text-zinc-500 font-medium max-w-[200px]">
                    Select a Template Blueprint to configure the generation
                    engine.
                  </p>
                </div>
              )}
