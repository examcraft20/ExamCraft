...
import type { DepartmentRecord, CourseRecord, SubjectRecord } from "../academic";
...
function createInitialDemoState(): DemoState {
  ...
  return {
    user: {
      ...
      roleCodes: ["super_admin", "institution_admin", "academic_head", "reviewer_approver", "faculty"],
      isSuperAdmin: true
    },
    memberships: [
      {
        ...
        roleCodes: ["institution_admin", "academic_head", "reviewer_approver", "faculty"]
      }
    ],
    tenantContexts: {
      [demoInstitutionId]: {
    ...
    academic: {
      departments: [
        { id: "dept-1", institution_id: demoInstitutionId, name: "Science", code: "SCI", status: "active", created_at: now }
      ],
      courses: [
        { id: "course-1", institution_id: demoInstitutionId, department_id: "dept-1", name: "B.Sc Physics", code: "BSP", status: "active", created_at: now }
      ],
      subjects: [
        { id: "sub-1", institution_id: demoInstitutionId, department_id: "dept-1", course_id: "course-1", name: "Modern Physics", code: "PHY101", status: "active", created_at: now }
      ]
    },
    globalTemplates: [
      {
...
export async function demoApiRequest<TResponse>(
  ...
  if (method === "GET" && rawPath === "/tenant/context") {
    const institutionId = options.institutionId || demoState.memberships[0]?.institutionId;
  ...
  if (method === "POST" && rawPath === "/content/questions") {
    const created = {
      ...
      courseOutcomes: Array.isArray(body?.courseOutcomes) ? body.courseOutcomes : [],
  ...
  if (method === "POST" && rawPath === "/content/questions/bulk") {
    const createdArray = (body?.questions || []).map((q: any, i: number) => ({
      ...
      courseOutcomes: Array.isArray(q.courseOutcomes) ? q.courseOutcomes : [],
    ...
    addAuditEvent("questions.bulk_imported", `${createdArray.length} questions`);
    return createdArray as TResponse;
  }

  const questionReviewMatch = rawPath.match(/^\/content\/questions\/([^/]+)\/review$/);
  ...
  if (method === "POST" && rawPath.startsWith("/content/global-templates/") && rawPath.endsWith("/clone")) {
  ...
  const templateReviewMatch = rawPath.match(/^\/content\/templates\/([^/]+)\/review$/);
  ...
  const paperReviewMatch = rawPath.match(/^\/content\/papers\/([^/]+)\/review$/);
  ...
  const paperSubmitMatch = rawPath.match(/^\/content\/papers\/([^/]+)\/submit$/);
  ...
  if (method === "POST" && rawPath === "/people/invitations") {
    ...
    demoState.people.invitations = [invitation, ...demoState.people.invitations];
    addAuditEvent("invitation.created", `Invitation sent to ${invitation.email}`);
  ...
  if (rawPath === "/invitations/preview" && method === "GET") {
    ...
    const invitation = demoState.people.invitations.find((item) => item.rawToken === token);
    if (!invitation) {
    ...
    return {
      invitation: {
        ...
        institutionName: demoState.memberships[0]?.institutionName ?? "Demo Institute",
  ...
  if (rawPath === "/invitations/accept" && method === "POST") {
    ...
    const invitation = demoState.people.invitations.find((item) => item.rawToken === body.token);
    if (!invitation) {
      throw new Error("Invitation not found.");
    }
    ...
    addAuditEvent("invitation.created", `Invitation accepted by ${invitation.email}`);
    return {
      email: invitation.email,
      institutionId: demoState.memberships[0]?.institutionId ?? "inst-demo-1"
    } as TResponse;
  ...
  if (rawPath === "/onboarding/institution" && method === "POST") {
    ...
    demoState.tenantContexts[institutionId] = {
      institutionId,
      institutionUserId: demoState.memberships[demoState.memberships.length - 1].institutionUserId,
      roleCodes: ["institution_admin"],
    ...
    addAuditEvent("institution.created", `${institutionName} onboarded`);
    return {
      institution: { id: institutionId, name: institutionName, slug: institutionSlug },
  ...
  const platformStatusMatch = rawPath.match(/^\/tenant\/platform-institutions\/([^/]+)\/status$/);
  if (platformStatusMatch && method === "PATCH") {
    const institutionId = platformStatusMatch[1];
    const institution = demoState.platformInstitutions.find((item) => item.id === institutionId);
    if (!institution) {
      throw new Error("Institution not found.");
    }
    institution.status = body?.status || institution.status;
    addAuditEvent("institution.status_changed", `Status updated to ${institution.status}: ${institution.name}`);
  ...
  if (rawPath === "/tenant/branding" && method === "PATCH") {
    const institutionId = options.institutionId || demoState.memberships[0]?.institutionId;
    const membership = demoState.memberships.find(m => m.institutionId === institutionId);
    if (membership) {
      membership.branding = {
        ...membership.branding,
        ...body
  ...
  if (rawPath === "/academic/departments" && method === "POST") {
    const newDept: DepartmentRecord = {
      id: `dept-${Date.now()}`,
      institution_id: options.institutionId || demoState.memberships[0].institutionId,
      name: body?.name,
      code: body?.code || null,
      status: "active",
...
function buildInstitutionSummary(): InstitutionDashboardSummaryResponse {
  return {
    ...
    recentInvitations: demoState.people.invitations.slice(0, 5).map((invite) => ({
      id: invite.id,
      email: invite.email,
...
function buildPlatformSummary(): PlatformDashboardSummaryResponse {
  ...
  return {
    ...
    recentInstitutions: demoState.platformInstitutions.slice(0, 5).map((institution) => ({
      id: institution.id,
...
function addAuditEvent(eventType: PlatformAuditEvent["eventType"], title: string) {
