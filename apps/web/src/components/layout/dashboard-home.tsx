...
export function DashboardHome() {
  ...
  const [isOpeningInstitutionId, setIsOpeningInstitutionId] = useState<
    string | null
  >(null);
  ...
  const autoSelectedMembership = useMemo(() => {
    ...
    return (
      memberships.find(
        (membership) => membership.institutionId === preferredInstitutionId,
      ) ?? null
    );
    ...
    async function loadDashboardState() {
      ...
      try {
        const [authResponse, membershipResponse] = await Promise.all([
          ...
          apiRequest<MembershipSummary[]>("/institution/memberships", {
            method: "GET",
            accessToken: session.access_token,
          }),
        ]);
        ...
        if (isMounted) {
          setStatus(
            error instanceof Error
              ? error.message
              : "Unable to load your institutions.",
          );
        }
      } finally {
        if (isMounted) {
  ...
  const openMembership = useCallback(
    async (membership: MembershipSummary) => {
      ...
      try {
        const response = await apiRequest<InstitutionContextResponse>(
          "/institution/context",
          {
            method: "GET",
        ...
        const resolvedRole = resolvePrimaryRole(
          response.institutionContext.roleCodes,
        );
        if (!resolvedRole)
          throw new Error(
        ...
        setStatus(
          error instanceof Error
            ? error.message
            : "Unable to open this institution.",
        );
  ...
  useEffect(() => {
    if (
      !sessionState ||
      !autoSelectedMembership ||
      isOpeningInstitutionId ||
      errorOccurred
    )
      return;
      ...
      <div className="max-w-6xl mx-auto flex flex-col gap-12 relative">
        ...
        <header className="flex flex-col gap-2">
          ...
          <h1 className="text-5xl font-black tracking-tighter text-white drop-shadow-2xl">
            Welcome back to{" "}
            <span className="text-indigo-400 italic">ExamCraft.</span>
          </h1>
          <p className="text-zinc-500 font-medium max-w-xl">
            Select an authorized institution below to enter your dedicated role
            workspace. Contextual security is applied upon entry.
          </p>
        </header>

        {status ? (
          <StatusMessage variant="error" className="shadow-2xl">
            {status}
          </StatusMessage>
        ) : null}
        ...
        <section className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            <div className="col-span-full">
              <Card className="!bg-zinc-900 border-white/5 !rounded-[2.5rem] p-12 text-center flex flex-col items-center gap-6 shadow-2xl relative overflow-hidden group">
                ...
                <p className="text-zinc-500 max-w-md font-medium">
                  Your role-based pedagogical dashboards become available
                  immediately after identity verification.
                </p>
                <div className="flex gap-4 pt-4">
                  <Button
                    onClick={() => router.push("/login")}
                    className="bg-white text-black px-8 py-6 rounded-2xl font-black text-lg shadow-2xl hover:scale-105 active:scale-95 transition-all"
                  >
                    Sign In Now
                  </Button>
                  <Button
                    ...
                    className="px-8 py-6 rounded-2xl font-black text-lg"
                  >
                    Setup Institution
                  </Button>
                </div>
          ...
          {isLoading &&
            [1, 2, 3].map((i) => (
              <div
                key={i}
                className="animate-pulse flex flex-col gap-4 p-8 rounded-[2rem] bg-white/5 border border-white/5 h-[320px]"
              >
            ...
            !sessionState.isSuperAdmin && (
              <div className="col-span-full">
                <Card className="!bg-zinc-900 border-white/5 !rounded-[2.5rem] p-12 text-center flex flex-col items-center gap-6 shadow-2xl overflow-hidden group">
                  ...
                  <h2 className="text-3xl font-black tracking-tight text-white px-12">
                    No active workspace memberships found.
                  </h2>
                  <p className="text-zinc-500 max-w-sm font-medium">
                    Create a digital campus or accept an automated invitation
                    from your coordinator to unlock your dashboard.
                  </p>
                  <div className="flex flex-wrap gap-4 pt-4 justify-center">
                    <Button
                      onClick={() => router.push("/onboarding")}
                      className="bg-indigo-600 text-white px-8 py-6 rounded-2xl font-black text-lg shadow-2xl hover:scale-105 active:scale-95 transition-all"
                    >
                      Launch Institution
                    </Button>
                    <Button
                      ...
                      className="px-8 py-6 rounded-2xl font-black text-lg hover:bg-white/10"
                    >
                      Accept Invite
                    </Button>
                  </div>
...
function InstitutionCard({
  ...
  return (
    ...
    >
      ...
      <div
        className={`absolute inset-0 bg-gradient-to-br transition-opacity opacity-0 group-hover:opacity-100 ${isSuperAdmin ? "from-violet-500/10 via-transparent" : "from-indigo-500/10 via-transparent"} -z-10`}
      />
      ...
      <div className="flex flex-col gap-2">
        ...
        <p className="text-zinc-500 text-xs font-medium leading-relaxed line-clamp-3">
          {isSuperAdmin
            ? "Manage global institution infrastructure and audit logs."
            : `Full access to orchestrate exam operations within the ${name} workspace.`}
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-2 mb-2">
        {roleLines.map((r: string) => (
          <span
            key={r}
            className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-indigo-500/10 border border-indigo-500/20 text-[9px] font-bold uppercase tracking-wider text-indigo-300"
          >
            {r.replace("_", " ")}
          </span>
        ))}
      ...
      <div className="flex items-center justify-between pt-4 border-t border-white/5 group-hover:border-white/10 transition-colors">
        <span
          className={`text-sm font-black uppercase tracking-widest ${isSuperAdmin ? "text-violet-400" : "text-white"}`}
        >
          {isOpening ? "Initializing..." : "Enter Workspace"}
        </span>
        <div
          className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all ${isSuperAdmin ? "bg-violet-600 text-white" : "bg-white text-black group-hover:bg-indigo-600 group-hover:text-white"}`}
        >
